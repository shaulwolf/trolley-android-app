...
try {
  const updateLastModified = db.prepare('UPDATE products SET lastModified = dateAdded WHERE lastModified IS NULL');
  const result1 = updateLastModified.run();
  
  const updateDeviceSource = db.prepare('UPDATE products SET deviceSource = ? WHERE deviceSource IS NULL');
...
`),
  updateProduct: db.prepare(`
    UPDATE products 
    SET title = ?, price = ?, originalPrice = ?, image = ?, site = ?, displaySite = ?, category = ?, variants = ?, lastModified = ?
    WHERE id = ?
  `),
