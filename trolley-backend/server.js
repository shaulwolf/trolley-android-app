...
app.post("/api/products", (req, res) => {
  try {
    ...
    if (existing) {
      console.log("⚠️ Product already exists:", url);
      return res.status(409).json({ error: "Product already exists" });
    }

    const insertResult = statements.insertProduct.run(
      id,
      url,
      title,
      price,
    ...
    console.log("✅ Product inserted successfully:", insertResult);
...
app.get("/api/sync", (req, res) => {
  try {
    ...
    console.log(
      `📥 Enhanced Sync GET request${since ? ` since ${since}` : ""}`
    );
    ...
    if (since) {
      const stmt = db.prepare(
        "SELECT * FROM products WHERE lastModified > ? ORDER BY lastModified DESC"
      );
    ...
    res
      .status(500)
      .json({ error: "Failed to get sync data", details: error.message });
  }
});
...
app.post("/api/sync", (req, res) => {
  try {
    ...
    console.log(
      `✅ Complete state sync completed: ${insertedCount} products inserted`
    );
    console.log(
      `🔄 Any products not in upload from ${deviceId} have been automatically deleted`
    );
...
app.get("/api/sync/status", (req, res) => {
  try {
    ...
    const newestProductStmt = db.prepare(
      "SELECT MAX(lastModified) as newest FROM products"
    );
...
async function scrapeProduct(url, retryCount = 0) {
  ...
  try {
    ...
    const baseDelay =
      2000 + retryCount * 1000 + Math.floor(Math.random() * 2000);
    ...
    if (isDynamicSite) {
      ...
      try {
              ...
              try {
                ...
                for (const element of elements) {
                  const text = await element.evaluate(
                    (el) => el.textContent || el.value || el.title
                  );
        ...
        console.log(
          "⚠️ Dynamic loading failed, continuing with regular extraction"
        );
      }
    }
    ...
    const result = await page.evaluate(() => {
        ...
        for (const selector of priceSelectors.current) {
          ...
          for (const element of elements) {
            ...
            if (price && isValidProductPrice(price)) {
              currentPrice = price;
              console.log(
                `✅ Found current price: ${price} from selector: ${selector}`
              );
              break;
            }
          }
      ...
      function selectBestPrice(prices) {
        ...
        if (prices.length === 1)
          return { current: prices[0].text, original: null };

        // Remove duplicates
        const unique = prices.filter(
          (price, index, arr) =>
            arr.findIndex((p) => Math.abs(p.value - price.value) < 0.01) ===
            index
        );

        if (unique.length === 1)
          return { current: unique[0].text, original: null };
        if (unique.length === 2) {
    ...
    console.log(
      `   Title: "${result.title}" (${result.title?.length || 0} chars)`
    );
    console.log(
      `   Price: ${result.price} (${
        result.originalPrice ? `was ${result.originalPrice}` : "no original"
      })`
    );
    ...
    console.error(
      `❌ Scraping error (attempt ${retryCount + 1}):`,
      error.message
    );
    ...
    if (
      retryCount < 2 &&
      (error.message.includes("timeout") || error.message.includes("net::"))
    ) {
